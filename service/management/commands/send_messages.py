...
from service.models import Distribution, Attemp
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if mailing_id is not None:
            ...
            if distribution is not None and distribution.start_time.replace(tzinfo=None) < datetime.now() < distribution.end_time.replace(tzinfo=None):
                ...
                for email in emails:
                    ...
                    Attemp.objects.create(attempt_time=attempt_time, status_2=status, server_response=server_response, mailing=distribution)
            else:
